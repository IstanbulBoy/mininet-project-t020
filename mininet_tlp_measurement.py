...
class router(Topo):
	...
	linkopts3 = dict(bw=0.1, delay='20ms')
		...
		print('Adding links')
		if(choice=='fast'):
                	self.addLink(h1,h2, **self.linkopts1)
                	self.addLink(h2,h3, **self.linkopts1)
		elif(choice=='moderate'):
			self.addLink(h1,h2, **self.linkopts2)
                	self.addLink(h2,h3, **self.linkopts2)
		elif(choice=='slow'):
			self.addLink(h1,h2, **self.linkopts3)
                	self.addLink(h2,h3, **self.linkopts3)
		else:
			self.addLink(h1,h2)
			self.addLink(h2,h3)
...
def nc6TCPd(h1, h2, h3, bfs, destfile):
	...
	print('Netcat6')
	h1.sendCmd('dd | nc6 -4 -x -l -p 7676 < /dev/zero &')
	
	dest='test'
	if(destfile != None):
		dest=destfile
	print('TCPDUMPing')
	h2.cmd('tcpdump -s 68 -w captures/'+ str(dest) +'.pcap -i h2-eth0 &')

	print('Netcat6 started!')	
	print h3.sendCmd('nc6 -4 --rev-transfer --buffer-size='
			 + str(bfs) + h1.IP() +' 7676 > /dev/null')
